...
df <- datos |>
  ...
  count(monto_cubre_necesidades) |>
  mutate(
    prop = n/sum(n),
    text = paste0(n," (",scales::percent(prop, accuracy = 0.1),")"),
    monto_cubre_necesidades = fct_relabel(monto_cubre_necesidades, \(x) str_wrap(x,20))
...
g
ggsave("qmd/images/fig-05.jpg", dpi = 300, height = 5.82, width = 7.65)
