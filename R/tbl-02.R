...
df <- datos |>
  mutate(
    provincia_actual = factor(provincia_actual, levels = orden_provincias, ordered = TRUE)
  ) |>
  count(provincia_actual, .drop = FALSE) |>
  mutate(
    prop = n/sum(n)
  )
...
tabla <- df |>
  ...
  gt(rowname_col = "provincia_actual") |>
  cols_label(
    n    = "Frecuencia (personas)",
    prop = "Porcentaje"
  ) |>
  tab_stubhead(label = md("**Provincia**")) |>
  tab_header(
    title = md("**Costa Rica: Estudiantes del curso Análisis Exploratorio de Datos de la carrera de Estadística de la Universidad de Costa Rica según provincia de residencia actual, junio 2025**"
    ),
