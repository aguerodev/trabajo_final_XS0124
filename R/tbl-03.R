...
tabla <- df |>
  ...
  gt(rowname_col = "categoria_beca") |>
  cols_label(
    n    = "Frecuencia (personas)",
    prop = "Porcentaje"
  ) |>
  tab_stubhead(label = md("**Categoria de beca**")) |>
  tab_header(
    title = md("**Costa Rica: Estudiantes del curso Análisis Exploratorio de Datos de la Universidad de Costa Rica según categoría de beca socioeconómica, junio 2025**"),
