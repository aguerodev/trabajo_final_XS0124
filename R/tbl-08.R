...
df <- datos |>
  ...
  count(acuerdo_periodos, .drop = FALSE) |>
  mutate(
    prop = n/sum(n)
  )
...
tabla <- df |>
  ...
  gt(rowname_col = "acuerdo_periodos") |>
  cols_label(
    n    = "Frecuencia (personas)",
    prop = "Porcentaje"
  ) |>
  tab_stubhead(label = md("**Periodo de deposito**")) |>
  tab_header(
    title = md("**Costa Rica: Estudiantes del curso Análisis Exploratorio de Datos de la Universidad de Costa Rica según preferencia de periodo de depósito de la beca socioeconómica, junio 2025**"),
