...
df <- datos |>
  ...
  mutate(
    provincia_reubicacion = fct_lump_min(provincia_reubicacion, min = 2, other_level = "Otra")
  ) |>
  count(provincia_reubicacion) |>
  drop_na() |>
  mutate(
    prop = n/sum(n)
  )
...
tabla <- df |>
  ...
  gt(rowname_col = "provincia_reubicacion") |>
  cols_label(
    n    = "Frecuencia (personas)",
    prop = "Porcentaje"
  ) |>
  tab_stubhead(label = md("**Provincia de reubiación**")) |>
  tab_header(
